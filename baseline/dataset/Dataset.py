    ...
    def __init__(
        ...
        **kwargs):
        if os.path.exists(dataset):
            dataset1 = pickle.load(open(dataset, 'rb'))
            dataset2 = pickle.load(open('./dataset/peta/peta_dataset.pkl', 'rb'))
            # print('*'*100)
            # print(dataset)
            # print('./dataset/peta/peta_dataset.pkl')
            self.dataset = dict(dataset1, **dataset2)
        else:
            print(dataset + ' does not exist in dataset.')
            raise ValueError
        if os.path.exists(partition):
            part1 = pickle.load(open(partition, 'rb'))
            part2 = pickle.load(open('./dataset/peta/peta_partition.pkl', 'rb'))
            # print('*'*100)
            # print(partition)
            # print('./dataset/peta/peta_partition.pkl')
            self.partition = dict(part1, **part2)
        else:
